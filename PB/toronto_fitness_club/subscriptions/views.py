...
class ChooseSubscriptionAPIView(APIView):
    ...
    def post(self, request):
        ...
        if auth_token:
            ...
            if request.data['price'] == '' or request.data['length'] == '':
                return response.Response({'message': "invalid payload"},
    ...
    def put(self, request, pk):
        ...
        if auth_token:
            ...
            if len(request.data['card_number']) != 16:
                return response.Response({'card_number': "invalid card number, make sure it is 16 characters in length"},
                                         status=status.HTTP_400_BAD_REQUEST)
            if len(request.data['expiry_month']) != 2:
                return response.Response({'expiry_month': "invalid expiry month, make sure it is in the format MM"},
                                         status=status.HTTP_400_BAD_REQUEST)
            if len(request.data['expiry_year']) != 2:
                return response.Response({'expiry_year': "invalid expiry year, make sure it is in the format YY"},
                                         status=status.HTTP_400_BAD_REQUEST)
            if len(request.data['cvc']) != 3:
                return response.Response({'cvc': "invalid cvc, make sure it is 3 characters in length"},
                                         status=status.HTTP_400_BAD_REQUEST)
