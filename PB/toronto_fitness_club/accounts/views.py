    ...
    def post(self, request):
        ...
        if request.data['password'] != '' and request.data['password'] != '':
            ...
            if len(request.data['password']) < 8:
                x["password"] = ["password must be at least 8 characters in length"]
            ...
            if '~' not in request.data['password'] and '!' not in request.data['password'] and \
                ...
                x["password"] = ["password must be contain at least one of ~!@#$%&_"]
            ...
            if count_upper < 1:
                x["password"] = ["password needs to contain at least 1 upper case letter"]
...
class LoginAPIView(APIView):
    ...
    def post(self, request):
        ...
        if user:
            ...
            responsee.set_cookie(key='auth_token', value=serializer.data["auth_token"], httponly=True)
